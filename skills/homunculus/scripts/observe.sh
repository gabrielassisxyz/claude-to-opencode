#!/bin/bash
# Homunculus v2 Observation Capture (yaml-hooks adapted)
...
OBS_FILE="$PROJECT_DIR/.opencode/homunculus/observations.jsonl"
...
case "$EVENT_TYPE" in
  prompt)
    ...
    PROMPT=$(echo "$INPUT" | jq -r '.text // empty' 2>/dev/null || echo "")
    if [ -n "$PROMPT" ]; then
